...
def make_model(device) -> nn.Module:
    if configs.model.name == 'spvnas':
        ...
        model = SPVNAS(
            ...
            vres=net_config['vres']).to(torch.device('cuda:'+ str(device)))

        model.manual_select(net_config)
        model = model.determinize(local_rank=device)

        my_model_dict = model.state_dict()

        pre_weight = torch.load('SemanticKITTI_val_SPVNAS@65GMACs/init', map_location='cuda:'+ str(device))['model']
        part_load = {}
        match_size = 0
        nomatch_size = 0
