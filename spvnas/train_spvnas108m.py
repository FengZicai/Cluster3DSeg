...
def main() -> None:
    ...
    parser.add_argument('--run-dir', default='output/path/spvnas108m/', help='run directory')
    parser.add_argument('--distributed', default=False)
    parser.add_argument('--amp_enabled', default=True)
    args, opts = parser.parse_known_args()
    opts = ['--run-dir','output/path/spvnas108m/','--distributed','False', '--amp_enabled', 'True']
    ...
    model = builder.make_model().to(pytorch_device)
    if configs.distributed:
